...
class SqlalchemyTicketsFilter(TicketsFilter):
    ...
    async def build_query(self) -> Select:
        ...
        if self.origin_airport_ids:
            query &= and_(TicketSegmentOrm.origin_airport_id.in_(self.origin_airport_ids))

        if self.destination_airport_ids:
            query &= and_(TicketSegmentOrm.destination_airport_id.in_(self.destination_airport_ids))
