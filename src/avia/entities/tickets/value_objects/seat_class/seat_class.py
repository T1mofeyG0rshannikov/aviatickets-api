...
class SeatClass:
    ...
    CLASSES = [seat.value for seat in SeatClassEnum]

    def __post_init__(self):
        if self.value not in self.CLASSES:
            raise InvalidSeatClassError(
                f""""{self.value}" is invalid seat class. Choices is {", ".join(map(lambda f: f'"{f}"', self.CLASSES))}"""
            )
